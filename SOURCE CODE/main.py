...
small_font_size = 20
small_start_font = pygame.font.Font("PressStart2P-Regular.ttf", small_font_size)
...
def handle_turn(player):
    coin_values = [1, 2, 3]
    coin_probabilities = [0.35, 0.25, 0.15]
...
def display_homepage():
    ...
    while True:
        ...
        #pygame.draw.rect(screen, GRAY, title_rect, 2)
        screen.blit(title_text, title_rect)
        title_text = small_start_font.render("New Legacy", True, (255, 255, 255))
        ...
        # Sort the match history based on match number
        sorted_history = sorted(match_history, key=lambda x: int(x.split(":")[1].split()[1]))

        # Display sorted match history
